...
import { Account, Chat, ClockOutline, Cog, FolderOpenOutline, MusicCircleOutline, MusicClefTreble, Phone, Snake, VideoBox } from '../../assets/icons'
...
const Home = () => {
  ...
  const apps: App[][] = [
    ...
    [
      ...
      { id: 'ad1c1b9d-f47f-48d7-8659-a2453ffb4a77', icon: ClockOutline, click: () => setScreen(Screens.clock) },
      { id: 'baedb4ac-61c0-4d9f-a696-0fd3b1282be0', icon: Cog, click: () => setScreen(Screens.settings) },
    ],
    [
      { id: '2e8eea4c-babf-4922-aa6e-ed8ac5803604', icon: undefined, click: undefined },
      { id: 'c1c9620f-b9da-4478-a955-ae94174bc183', icon: Snake, click: () => setScreen(Screens.messenger) },
      { id: 'ecc4fb15-4e84-415b-8b20-9bd2b6707dea', icon: MusicClefTreble, click: () => setScreen(Screens.messenger) },
      ...
      {apps.map((row, idx) => (
        <div key={idx} className="home__row">
          {row.map(app => {
            const Icon = app.icon
            return (
              <div key={app.id} className={["home__col", `home__col--${darkMode}`].join(" ")}>
                {Icon && (
                  <button
                    onClick={app.click}
                    aria-label='phone'
